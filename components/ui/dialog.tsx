...
function DialogContent({
  ...
  function hasSlot(node: React.ReactNode, slotName: string): boolean {
    const arr = React.Children.toArray(node)
    for (const child of arr) {
      if (!React.isValidElement(child)) continue
      const props = (child as React.ReactElement<any>).props as any
      if (props['data-slot'] === slotName) return true
      if (hasSlot(props.children, slotName)) return true
    }
    return false
  }
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        {...props}
      >
        {/* Provide hidden defaults only when caller hasn't supplied them */}
        {!hasTitle && (
